...
function init() {
    ...
    for (let i = 0; i < refining_resource_list.length; i++) {
        ...
        if(refining_resource_list[i].tier > 4 || refining_resource_list[i].enchant > 0) {
            // make it rectangular in size
            let transmutation_button = document.createElement("button");
            transmutation_button.className = "transmutation-button";
            transmutation_button.id = "transmutation-button-" + refining_resource_list[i].tier + "." + refining_resource_list[i].enchant;
            transmutation_button.innerHTML = "‎ ‎";
            transmutation_button.title = getCheapestTransmuteCost(refining_resource_list[i].tier, refining_resource_list[i].enchant);
            transmutation_button.onclick = function () {
                transmute(this);
            }


            resource_price_input.appendChild(transmutation_button);
        }
...
function loadFromLocalStorage(type) {
    ...
    updateElementFromLocalStorage("global-discount", false);
...
function updateNumbers(element) {
    ...
    for (let i = 0; i < table.rows.length; i++) {
        ...
        console.log(resource_cost +  " & " + product_price);
...
function getResourceCosts() {
    ...
    for (let i = 0; i < table.rows.length; i++) {
        ...
        if(i > 2) {
            resource_price = +table.rows[i].cells[2].children[1].value;
        }
...
function pullCurrentPrices(resources) {
    ...
    .then(data => {
        for(let i = 0; i < table.rows.length; i++) {
            ...
            if(resources && index > 2) {
                price_cell = table.rows[i].cells[4-offset].children[1]
            }
...
function getCheapestTransmuteCost(tier, enchant) {
    var t4_cost = document.getElementById("resource-price-input-4.0").value;
...
function transmute(element) {
    var enchant = element.id.replace("transmutation-button-", "")[0];
    var tier = element.id.replace("transmutation-button-", "")[2];
    var input_element = document.getElementById("resource-price-input-" + enchant + "." + tier);
    
    input_element.value = getCheapestTransmuteCost(enchant, tier);
    input_element.style.borderBottomColor = "blue";

    updateNumbers();
}

function updateTransmutationButtons() {
    var transmutation_buttons = document.getElementsByClassName("transmutation-button");
    for (var i = 0; i < transmutation_buttons.length; i++) {
        var element = transmutation_buttons[i];
        var element_id = element.id;
        
        var market_value = document.getElementById(element_id.replace("transmutation-button-", "resource-price-input-")).value;
        var transmutation_cost = getCheapestTransmuteCost(element_id.replace("transmutation-button-", "")[0], element_id.replace("transmutation-button-", "")[2]);

        if(market_value != 0 && market_value-1 < transmutation_cost) {
